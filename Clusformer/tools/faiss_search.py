...
from tools.faiss_gpu import faiss_search_approx_knn
...
def precise_dist(feat, nbrs, num_process=4, sort=True, verbose=False):
    ...
    feat_share = torch.from_numpy(feat)#.share_memory_()
    nbrs_share = torch.from_numpy(nbrs)#.share_memory_()
    dist_share = torch.zeros_like(nbrs_share, dtype=torch.float32)#.share_memory_()

    process_unit = 4000
    num, cols = nbrs_share.shape
    
    for i in tqdm(range(0, num, process_unit)):
        si = i
        ei = min(num, si + process_unit)

        batch_nbr = nbrs_share[si:ei]
        query = feat_share[si:ei].unsqueeze(1)
        gallery = feat_share[batch_nbr].permute(0, 2, 1)

        batch_sim = torch.bmm(query, gallery).view(-1, cols)

        if sort:
            batch_sim, indices = torch.sort(batch_sim, descending=True)
            batch_nbr = torch.gather(batch_nbr, 1, indices)
            nbrs_share[si:ei] = batch_nbr
        
        # import pdb; pdb.set_trace()
        dist_share[si:ei] = 1. - batch_sim
    # import pdb; pdb.set_trace()
